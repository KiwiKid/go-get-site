...
func pages(pages []Page, website Website, count LinkCountResult, pageUrl string, prevPage string, nextPage string, addedPagesSet map[string]struct{}, progress string, viewPageSize int, processPageSize int, dripLoad bool, dripLoadCount int, dripLoadFreqMin int, dripLoadStr string, processAll bool, skipNewLinkInsert bool, ignoreWarnings bool, attributeSets []AttributeSet, selectedAttributeSetId uint, message string) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) (err error) {
		...
		_, err = templBuffer.WriteString("</option></select></div></details></form></div><div><button hx-get=\"")
		if err != nil {
			return err
		}
		_, err = templBuffer.WriteString(templ.EscapeString(websiteBlocksURL(website.ID)))
		if err != nil {
			return err
		}
		_, err = templBuffer.WriteString("\">")
		...
		var_45 := `Start Loading Blocks`
		_, err = templBuffer.WriteString(var_45)
		if err != nil {
			return err
		}
		_, err = templBuffer.WriteString("</button></div><div class=\"flex mb-2 mt-4 font-bold text-sm text-gray-600 border-b-2\"><div class=\"w-1/4 p-2\">")
		if err != nil {
			return err
		}
		var_46 := `Status`
		...
		if selectedAttributeSetId == 0 {
			...
			var var_53 string = strconv.Itoa(int(selectedAttributeSetId))
